...
/**************** impg direct construction helpers ****************/
...
static void impgDestroySideIfNeeded (NodeSide *side, U8 status, U8 simpleBit)
{
  if ((status & NODE_EXISTS) && !(status & simpleBit)) rsDestroy (side->rs) ;
}

static void impgSetNodeSide (
  NodeSide *side,
  U8 *status,
  U8 simpleBit,
  const ImpgSyngEdge *edges,
  I32 nEdge,
  const I64 *runSym,
  const I64 *runLen,
  I64 nRun
)
{
  if (!nEdge) return ;
  if (nEdge == 1 && nRun <= 1)
    { side->sync = edges[0].sync ;
      side->offset = edges[0].offset ;
      side->count = edges[0].count ;
      *status |= simpleBit ;
      return ;
    }

  I32 *symbol = new (nEdge, I32) ;
  U32 *offset = new (nEdge, U32) ;
  for (I32 i = 0 ; i < nEdge ; ++i)
    { symbol[i] = edges[i].sync ; offset[i] = edges[i].offset ; }
  side->rs = rsBuildFixedSyng (nEdge, symbol, offset, nRun, (I64*)runSym, (I64*)runLen) ;
  newFree (symbol, nEdge, I32) ;
  newFree (offset, nEdge, U32) ;
}

void impg_syngBWTsetNode (
  SyngBWT *sb,
  I32 nodeId,
  const ImpgSyngEdge *inEdges,
  I32 nInEdge,
  const I64 *inRunSym,
  const I64 *inRunLen,
  I64 nInRun,
  const ImpgSyngEdge *outEdges,
  I32 nOutEdge,
  const I64 *outRunSym,
  const I64 *outRunLen,
  I64 nOutRun
...
{
  if (!sb || nodeId <= 0) return ;
  Node *node = arrayp (sb->node, nodeId, Node) ;
  U8   *status = arrayp (sb->status, nodeId, U8) ;
  impgDestroySideIfNeeded (&node->in, *status, NODE_SIMPLE_IN) ;
  impgDestroySideIfNeeded (&node->out, *status, NODE_SIMPLE_OUT) ;
  memset (node, 0, sizeof(Node)) ;
  *status = 0 ;
  if (!nInEdge && !nOutEdge) return ;

  *status = NODE_EXISTS ;
  impgSetNodeSide (&node->in, status, NODE_SIMPLE_IN,
		   inEdges, nInEdge, inRunSym, inRunLen, nInRun) ;
  impgSetNodeSide (&node->out, status, NODE_SIMPLE_OUT,
		   outEdges, nOutEdge, outRunSym, outRunLen, nOutRun) ;
  if (DEBUG && nodeCheck (node, *status)) die ("impg_syngBWTsetNode nodeCheck failed for %d", nodeId) ;
}

void impg_syngBWTstartCountAdd (SyngBWT *sb, I32 startNode, U32 count)
{
  if (count) startCountAdd (sb, startNode, count) ;
}

void impg_syngBWTlocBuild (SyngBWT *sb)
{
  syngBWTlocBuild (sb) ;
}
